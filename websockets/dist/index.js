...
wss.on("connection", function connection(ws) {
    ws.on("message", function message(message) {
        ...
        if (res.type == "joinRoom") {
            const user = new User_1.User(res.data.userID, { x: Math.floor(Math.random() * 20), y: Math.floor(Math.random() * 20) }, ws);
            ...
            if (RoomManager_1.RoomManager.getInstance().checkMovement(res.data.roomID, res.data.userID, { x: res.data.x, y: res.data.y })) {
                ...
                RoomManager_1.RoomManager.getInstance().broadcast("rejected", res.data.roomID, res.data.userID);
                RoomManager_1.RoomManager.getInstance().displayRooms();
            }
        }
    });
