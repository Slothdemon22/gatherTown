...
import { Coordinates } from "./classes/types";
...
wss.on("connection", function connection(ws: WebSocket) {
    ...
    ws.on("message", function message(message:BinaryType)
    {
        ...
        {
            const user = new User(res.data.userID, { x: Math.floor(Math.random() * 20), y: Math.floor(Math.random() * 20) },ws);
            RoomManager.getInstance().addUser(res.data.roomID, user);

            ws.send(JSON.stringify({ Status: `Joined Room ${res.data.roomID}` }));
            RoomManager.getInstance().broadcast(res.type,res.data.roomID, res.data.userID);
        ...
        else if (res.type=='movement')
        {
           // console.log("moevment event ", res.type);
            if (RoomManager.getInstance().checkMovement(res.data.roomID, res.data.userID, { x: res.data.x, y: res.data.y }))
            {
                //console.log("inside");
                RoomManager.getInstance().updateMovement(res.data.roomID, res.data.userID, { x: res.data.x, y: res.data.y });
                RoomManager.getInstance().broadcast(res.type,res.data.roomID,res.data.userID)
            }
            else 
            {
                RoomManager.getInstance().broadcast("rejected", res.data.roomID, res.data.userID);
                RoomManager.getInstance().displayRooms();
            }
        }
